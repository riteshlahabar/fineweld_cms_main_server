...
class ItemController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('items.item.create', compact('data', 'todaysDate'));
    }
    ...
    public function datatableList(Request $request)
    {
        ...
        $data = Item::with([
            ...
            'brand',
            'category',
            'itemGeneralQuantities' => function ($query) use ($warehouseId, $accessibleWarehouseIds) {
                $query->when($warehouseId, function ($warehouseQuery) use ($warehouseId) {
                    return $warehouseQuery->where('warehouse_id', $warehouseId);
                })
                    ->when(! $warehouseId && $accessibleWarehouseIds->isNotEmpty(), function ($warehouseQuery) use ($accessibleWarehouseIds) {
                        return $warehouseQuery->whereIn('warehouse_id', $accessibleWarehouseIds);
                    });
            },
        ...
        return DataTables::of($data)
            ...
            ->addColumn('warehouse_name', function ($row) use ($warehouseId, $selectedWarehouseName, $accessibleWarehouseIds) {
                $itemWarehouseStock = $row->itemGeneralQuantities ?? collect();

                if ($warehouseId) {
                    return $selectedWarehouseName;
                }

                if ($accessibleWarehouseIds->isNotEmpty()) {
                    $itemWarehouseStock = $itemWarehouseStock->whereIn('warehouse_id', $accessibleWarehouseIds);
                }
