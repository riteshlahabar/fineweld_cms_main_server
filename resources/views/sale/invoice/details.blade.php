                    ...
                    <div class="card-body">
                        <div class="toolbar hidden-print">
                                <div class="text-end">
                                    ...
                                    @if(config('services.einvoice.auth_url'))
                                        <form action="{{ route('sale.invoice.einvoice.generate', ['id' => $sale->id]) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-outline-success px-4">
                                                <i class="bx bx-receipt"></i> Generate IRN
                                            </button>
                                        </form>
                                        <form action="{{ route('sale.invoice.einvoice.sync', ['id' => $sale->id]) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-outline-primary px-4">
                                                <i class="bx bx-refresh"></i> Sync IRN
                                            </button>
                                        </form>
                                        @if($sale->irn)
                                            <form action="{{ route('sale.invoice.einvoice.cancel', ['id' => $sale->id]) }}" method="POST" class="d-inline">
                                                @csrf
                                                <input type="hidden" name="reason_code" value="1">
                                                <input type="hidden" name="remark" value="Cancelled from CMS">
                                                <button type="submit" class="btn btn-outline-danger px-4">
                                                    <i class="bx bx-x-circle"></i> Cancel IRN
                                                </button>
                                            </form>
                                            <form action="{{ route('sale.invoice.ewaybill.generate', ['id' => $sale->id]) }}" method="POST" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn btn-outline-warning px-4">
                                                    <i class="bx bx-car"></i> Generate EWB
                                                </button>
                                            </form>
                                        @endif
                                        @if($sale->ewb_no)
                                            <form action="{{ route('sale.invoice.ewaybill.cancel', ['id' => $sale->id]) }}" method="POST" class="d-inline">
                                                @csrf
                                                <input type="hidden" name="reason_code" value="1">
                                                <input type="hidden" name="remark" value="Cancelled from CMS">
                                                <button type="submit" class="btn btn-outline-danger px-4">
                                                    <i class="bx bx-block"></i> Cancel EWB
                                                </button>
                                            </form>
                                        @endif
                                    @endif
                                    ...
                                    <main>
                                        <div class="row contacts">
                                            ...
                                            <div class="col invoice-details">
                                                ...
                                                @endif
                                                @if($sale->irn)
                                                    <div class="date">IRN: {{ $sale->irn }}</div>
                                                @endif
                                                @if($sale->irn_ack_no)
                                                    <div class="date">Ack No: {{ $sale->irn_ack_no }}</div>
                                                @endif
                                                @if($sale->irn_ack_date)
                                                    <div class="date">Ack Date: {{ $sale->irn_ack_date }}</div>
                                                @endif
                                                @if($sale->ewb_no)
                                                    <div class="date">EWB No: {{ $sale->ewb_no }}</div>
                                                @endif
                                                @if($sale->ewb_date)
                                                    <div class="date">EWB Date: {{ $sale->ewb_date }}</div>
                                                @endif
                                                @if($sale->ewb_valid_till)
                                                    <div class="date">EWB Valid Till: {{ $sale->ewb_valid_till }}</div>
                                                @endif
                                                @if($sale->einvoice_status)
                                                    <div class="date">GST Status: {{ strtoupper($sale->einvoice_status) }}</div>
                                                @endif
                                                @if($sale->einvoice_error)
                                                    <div class="date text-danger">GST Error: {{ $sale->einvoice_error }}</div>
                                                @endif
                                            </div>
                                        </div>
                                        @php
